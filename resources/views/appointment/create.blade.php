    ...
    <main class="dashboard-main">
        ...
        <div class="dashboard-content">
            <div class="content-container">
                ...
                <form action="{{ route('admin.appointment.store') }}" method="POST" class="appointment-form" id="appointmentForm">
                    @csrf
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            ...
                            <input type="hidden" name="patientID" id="patient_id">
                        </div>
                    </div>
                    ...
                    <div class="form-buttons">
                        ...
                        <a href="{{ route('admin.appointment.list') }}" class="btn btn-secondary">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ===========================================
    // 1. AUTO-FILL PATIENT INFO FUNCTIONALITY
    // ===========================================
    ...
    if (patientSelect) {
        patientSelect.addEventListener('change', function () {
            ...
            patientTelInput.value = '';
            patientIdInput.value = '';
            
            // Fill in the data if a patient is selected
    ...
    // ===========================================
    // 2. DATE AND TIME SLOT FUNCTIONALITY
    // ===========================================
    ...
    appointmentDateInput.addEventListener('change', function () {
        if (validateAppointmentDate()) {
            ...
            // Clear time slots if invalid date
    ...
    function updateAvailableTimeSlots() {
        ...
        // Define fixed time slots (2-hour appointments)
        const fixedSlots = [
            { start: 9, end: 11 },   // 9:00 AM - 11:00 AM
            { start: 11, end: 13 },  // 11:00 AM - 1:00 PM
            { start: 13, end: 15 },  // 1:00 PM - 3:00 PM
            { start: 15, end: 17 }   // 3:00 PM - 5:00 PM
        ];

        // Show loading state
        scheduleDisplay.innerHTML = '<p class="text-muted">Loading available time slots...</p>';
        appointmentTimeSelect.innerHTML = '<option value="">Loading...</option>';

        // Fetch booked slots from the server
        fetch(`{{ url('admin/appointment/check-slots') }}/${selectedDate}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(bookedSlots => {
                // Clear existing options and schedule display
                appointmentTimeSelect.innerHTML = '<option value="">Select Time</option>';
                scheduleDisplay.innerHTML = '<div class="schedule-slots"></div>';

                let hasAvailableSlots = false;

                // Process each fixed slot
                fixedSlots.forEach(slot => {
                    ...
                    const isBooked = bookedSlots.some(bookedSlot => bookedSlot.start === slot.start);
                    ...
                    slotDiv.className = `schedule-slot ${isBooked ? 'booked' : 'available'}`;
                    slotDiv.innerHTML = `
                        <span class="slot-time">${label}</span>
                        <span class="slot-status">${isBooked ? 'Booked' : 'Available'}</span>
                    `;

                    // Add click functionality for available slots
                    if (!isBooked) {
                    ...
                    scheduleDisplay.querySelector('.schedule-slots').appendChild(slotDiv);
                });

                // Show message if no slots available
                if (!hasAvailableSlots) {
                    scheduleDisplay.innerHTML = '<p class="text-danger">No available time slots for this date.</p>';
                }
            })
            .catch(error => {
                console.error('Error fetching time slots:', error);
                scheduleDisplay.innerHTML = '<p class="text-danger">Error loading time slots. Please try again.</p>';
