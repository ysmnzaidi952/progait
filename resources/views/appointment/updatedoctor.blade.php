                ...
                <div class="appointment-detail-header">
                    ...
                    <div class="appointment-meta">
                        ...
                        <span class="appointment-status">
                            ...
                            @endphp
                            <i class="fas {{ $statusIcon }}"></i> Status: <span class="{{ $statusClass }}-text">{{ ucfirst($status) }}</span>
                        </span>
                    </div>
                </div>

                <form action="{{ route('doctor.appointment.update', $appointment->appID ) }}" method="POST" class="appointment-form" id="appointmentForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control" id="medicalNotes" name="medicalNotes" rows="3">{{ $appointment->medicalNotes }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-row" id="completionNotesContainer" style="{{ $appointment->status == 'completed' ? '' : 'display: none;' }}">
                        <div class="form-group col-md-12">
                            <label for="completionNotes">Completion Notes</label>
                            <textarea class="form-control" id="completionNotes" name="completionNotes" rows="3">{{ $appointment->completionNotes }}</textarea>
                        </div>
                    </div>

                    <div class="form-row" id="cancellationReasonContainer" style="{{ $appointment->status == 'cancelled' ? '' : 'display: none;' }}">
                        <div class="form-group col-md-12">
                            <label for="cancelReason">Cancellation Reason</label>
                            <textarea class="form-control" id="cancelReason" name="cancelReason" rows="3">{{ $appointment->cancelReason }}</textarea>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const completionNotesContainer = document.getElementById('completionNotesContainer');
    ...
    statusSelect.addEventListener('change', function() {
        ...
        if (selectedStatus === 'completed') {
            completionNotesContainer.style.display = '';
        } else {
            completionNotesContainer.style.display = 'none';
        }

        // Show/hide cancellation reason
        if (selectedStatus === 'cancelled') {
            cancellationReasonContainer.style.display = '';
        } else {
    ...
    function updateAvailableTimeSlots() {
        ...
        fetch(`/doctor/appointment/check-slots/${selectedDate}?exclude=${currentAppointmentId}`)
